...
- **Juz (parah) browsing** — the same mushaf walked by its thirty ajza instead of its 114 surahs,
  ...
  `pwa-website/data/quran/juz.json` are generated by `tools/build-quran-juz.mjs` from two
  independent sources and verified to tile all 6,236 ayahs exactly; `test/quran-juz.test.js` keeps
  that property enforced.
