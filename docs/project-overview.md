...
- **Juz (parah) browsing** — the same mushaf walked by its thirty ajza instead of its 114 surahs,
  opening at each juz's own first ayah rather than at the top of the surah containing it, with a
  strip in the reader that tracks which juz the reading has reached. The boundaries in
  `pwa-website/data/quran/juz.json` are our own canonical data, served from our own origin and
  precached -- the app asks no third party where a juz begins. `tools/verify-quran-juz.mjs` checks
  that file rather than generating it: offline it proves the thirty ajza tile all 6,236 ayahs of the
  mushaf we ship, and `--online` corroborates them against quran.com and alquran.cloud, which can
  only ever disagree and never supply. The offline check runs in `npm run data:safety`, and
  `test/quran-juz.test.js` enforces the same invariant at test time.
...
- **Qibla** — great-circle bearing plus a live compass that works in any pose. The device's top
  ...
  points (roll-invariant, so landscape is fine). Readings with no north reference are refused
  rather than pointed somewhere confidently wrong, the screen's own rotation is accounted for in
  the flat pose, and the dial is smoothed and never spins the long way round at north. When no
...
- **Install shortcuts** — the installed app's icon offers Prayer Times, Qibla, Tasbih and Quran
  (`manifest.json` `shortcuts`), each opening `./?screen=<name>`. Handled at boot by
  `shortcutScreen()` in `routes.js` against an allow-list, deliberately separate from the
  canonical content routes. Icons generated by `tools/build-shortcut-icons.mjs`.
...
- **PWA identity** — a shared gold crescent/gateway mark with a simplified favicon, separate
  ...
  shows are precached; the install-only ones are not, since the OS reads them at install time.
