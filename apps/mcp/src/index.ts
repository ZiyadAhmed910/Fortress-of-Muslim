...
export default {
  async fetch(request: Request, env: Bindings): Promise<Response> {
    ...
    if (url.pathname === '/health') return Response.json({ status: 'ok', service: 'fortress-mcp', environment: env.PLATFORM_ENV, version: '0.8.1', timestamp: new Date().toISOString() });
    ...
    if (message.method === 'initialize') return rpc(message.id, { protocolVersion: '2025-06-18', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'Fortress Platform MCP', version: '0.8.1' }, instructions: 'Use Fortress tools for authoritative dua content. All currently exposed tools are read-only. Prefer search_duas to discover records and get_dua for the complete text.' });
