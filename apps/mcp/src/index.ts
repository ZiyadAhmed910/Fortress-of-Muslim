...
export default {
  async fetch(request: Request, env: Bindings): Promise<Response> {
    ...
    if (url.pathname === '/health') return Response.json({ status: 'ok', service: 'fortress-mcp', environment: env.PLATFORM_ENV, version: '0.9.0', timestamp: new Date().toISOString() });
    ...
    if (message.method === 'initialize') return rpc(message.id, { protocolVersion: '2025-06-18', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'Fortress Platform MCP', version: '0.9.0' }, instructions: 'Use Fortress tools for authoritative dua content. All tools are read-only. For a named dua or situation, call find_dua first because it fuzzy-matches titles and returns complete records in one call. Use search_duas only for broad searches inside the dua text.' });
