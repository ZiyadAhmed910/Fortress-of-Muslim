...
## 2. Prefetch the codebase map at the start of every session

Before doing anything else in a session that will touch code, run:

```powershell
npm run map:build
```

This regenerates `docs/codebase-map/{README.md,index.json}` — a machine-generated index of every
source file's exports/imports, every HTTP route across every Worker, and a reverse index of every
SQL table/view -> which files actually reference it. The generator is idempotent (it reuses the
previous `generatedAt` timestamp and touches nothing on disk when nothing structural changed), so
running it on a whim costs a few seconds and produces zero diff noise when the repo hasn't moved.

Read `docs/codebase-map/README.md` first when you need to find where something lives, instead of
grepping/exploring from scratch — it's a few hundred lines instead of thousands. It is **not** a
substitute for reading the actual file before changing it, and it does not explain *why* code does
what it does — it only tells you *where*.
...
Regenerate it again (same command) after any change that adds/removes/renames a file, route, or SQL
table, before ending the session — a stale map is worse than no map because it looks authoritative.
